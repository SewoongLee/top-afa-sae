...
class BatchTopKSAE(BaseAutoencoder):
    ...
    def get_loss_dict(self, x, x_reconstruct, acts, acts_topk, x_mean, x_std):
        ...
        afa_loss = self.cfg['afa_coeff'] * torch.mean((torch.norm(acts_topk, p=2, dim=-1) - torch.norm(x, p=2, dim=-1)).pow(2))
        loss = l2_loss + l1_loss + aux_loss + afa_loss
        num_dead_features = (
            self.num_batches_not_active > self.cfg["n_batches_to_dead"]
        ).sum()
        sae_out = self.postprocess_output(x_reconstruct, x_mean, x_std)
        dec_off_diag = (self.W_dec @ self.W_dec.T)[~torch.eye(self.W_dec.shape[0], dtype=torch.bool, device=self.W_dec.device)]
        output = {
            "sae_out": sae_out,
            ...
            "aux_loss": aux_loss,
            "afa_loss": afa_loss,
            "epsilon_max": dec_off_diag.max(),
            "epsilon_mean": dec_off_diag.mean(),
        }
        return output
...
class TopAFASAE(BaseAutoencoder):
    def __init__(self, cfg):
        super().__init__(cfg)
    ...
    def forward(self, x):
        ...
        x_cent = x - self.b_dec
        acts = F.relu(x_cent @ self.W_enc) # Shape: (batch_size, h)
        
        afa = torch.norm(x_cent, p=2, dim=1, keepdim=True).pow(2)
        
        dec_scaled_acts = (acts * torch.norm(self.W_dec, p=2, dim=-1)).pow(2)
        ...
        cumulative_fa[..., -1] = 1e8 # Ensure the last cumulative norm is large enough

        afa = torch.sqrt(afa)
        cumulative_fa = torch.sqrt(cumulative_fa)
        
        k = torch.abs(cumulative_fa - afa).argmin(dim=-1) + 1 # where this difference is smallest

        acts_topk = acts * torch.zeros_like(acts, dtype=torch.bool).scatter_(
            dim=1, 
            index=max_indices, 
            src=torch.arange(acts.shape[1], device=acts.device).unsqueeze(0) < k.unsqueeze(1), # Mask
        )
